...
void SEN5XComponent::setup() {
  ...
  // Der Sensor braucht etwas Zeit bis idle
  this->set_timeout(2000, [this]() {
    ...
    // Wenn der Sensor noch in einem „komischen“ Zustand ist: Reset.
    // Danach muss laut Datenblatt 200 ms gewartet werden, bevor neue Commands kommen.
    if (raw_read_status) {
      ESP_LOGD(TAG, "Sensor has data available, doing reset");
      ...
      // 200 ms NICHT-blockierend warten, dann weiter
      this->set_timeout(200, [this]() {
        this->set_timeout(1000, [this]() {  // zusätzlicher Puffer wie vorher (insg. ~1200ms)
          // --- ab hier identisch zur ursprünglichen Init-Sequenz ---
          ...
          // ⭐ Messung starten (SEN66: immer volle Messung)
      ...
      return;  // wichtig: wir machen den Rest über timeouts
    }

    // --- wenn kein Reset nötig war: direkt normal weiter ---
...
void SEN5XComponent::update() {
  ...
  // Wenn Messung gestoppt: alle Werte auf Unbekannt
  ...
  this->set_timeout(20, [this]() {
    ...
    float pm_2_5 = (measurements[1] == 0xFFFF || measurements[0] == 0xFFFF) ? NAN
                                                                            : (measurements[1] - measurements[0]) / 10.0f;
    float pm_4_0 = (measurements[2] == 0xFFFF || measurements[1] == 0xFFFF) ? NAN
                                                                            : (measurements[2] - measurements[1]) / 10.0f;
    float pm_10_0 = (measurements[3] == 0xFFFF || measurements[2] == 0xFFFF) ? NAN
                                                                             : (measurements[3] - measurements[2]) / 10.0f;
    ...
    this->set_timeout(50, [this]() {
      ...
      auto conv = [](uint16_t v) -> float { return (v == 0xFFFF) ? NAN : (float) v; };

      if (this->nc_0_5_sensor_)
        this->nc_0_5_sensor_->publish_state(conv(nc05_u));
      if (this->nc_1_0_sensor_)
        this->nc_1_0_sensor_->publish_state(conv(nc10_u));
      if (this->nc_2_5_sensor_)
        this->nc_2_5_sensor_->publish_state(conv(nc25_u));
      if (this->nc_4_0_sensor_)
        this->nc_4_0_sensor_->publish_state(conv(nc40_u));
      if (this->nc_10_0_sensor_)
        this->nc_10_0_sensor_->publish_state(conv(nc100_u));
    });
  });
}
...
bool SEN5XComponent::read_number_concentration(uint16_t *nc05, uint16_t *nc10, uint16_t *nc25, uint16_t *nc40,
                                               uint16_t *nc100) {
  uint16_t raw[5];
